...
from .config import LIVE_DIM_MAPPING, LIVE_IND_MAPPING, DIM_VALUE_MAPPING, MODULE_CONFIG
...
class LiveService:
                ...
                try:
                    ...
                    for row in rows:
                        ...
                        for en_field, value_dict in dimensions.items():
                            cn_field = LIVE_DIM_MAPPING.get(en_field, en_field)
                        ...
                        for en_field, value_dict in metrics.items():
                            cn_field = LIVE_IND_MAPPING.get(en_field, en_field)
    ...
    def collect_data(
        ...
        base_params = {
            ...
            "dimensions": json.dumps(["anchor_id", "stat_time_day", "roi2_material_anchor_name"]),
            "filters": json.dumps([
            ...
            "metrics": json.dumps(list(LIVE_IND_MAPPING.keys())),
        ...
        all_data = []
        with ThreadPoolExecutor(max_workers=self.max_workers, thread_name_prefix="Live") as executor:
            futures = [
        ...
        dim_columns = list(LIVE_DIM_MAPPING.values())
        ind_columns = list(LIVE_IND_MAPPING.values())
        time_columns = ["千川UID", "时间"]
